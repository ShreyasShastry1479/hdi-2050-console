...
- U.S. Bureau of Labor Statistics. [Annual-average CPI-U series and 2025 data](https://www.bls.gov/pir/spm/spm_chart_2025data.htm).
- U.S. Federal Reserve. [Statement on Longer-Run Goals and Monetary Policy Strategy](https://www.federalreserve.gov/monetarypolicy/files/FOMC_LongerRunGoals_202508.pdf).
...
## 16. Economic Outlook 2050

The Economy 2050 workspace is generated by `build_economic_outlook_2050.py` and stored in `data/output/economic_outlook_2050.csv`. It covers the same 193 countries as the national HDI projection table.

The observed baseline uses the latest available 2022-2024 World Bank values for GDP per capita at purchasing-power parity (`NY.GDP.PCAP.PP.CD`) and GDP per capita at market exchange rates (`NY.GDP.PCAP.CD`). Missing observations are flagged in the output and replaced by a transparent proxy: PPP uses the HDI model's GNI-per-capita PPP anchor, while nominal GDP uses the median nominal-to-PPP ratio among the nearest observed economic peers. The model does not present a proxy as an observation.

For each country, the projected annual real GDP-per-capita growth rate combines:

- growth prospects and developing-country catch-up readiness;
- industrialization, technology, digital infrastructure, and future readiness;
- institutional efficiency, human-capital absorption, and the demographic dividend;
- workforce-share change and income convergence headroom;
- negative adjustments for resource dependence, growth volatility, dependency pressure, and ageing.

The structural growth equation produces an independent annual growth signal bounded between -0.8% and 5.5%. The constant-2021 international-dollar 2050 PPP-per-capita value is reconciled to the national model's 2050 income index by exactly inverting the same logarithmic transformation:

`GDPpc_PPP_2050_2021$ = exp(ln(100) + IncomeIndex_2050 x (ln(105000) - ln(100)))`

The reported 2025-2050 real per-capita CAGR is the rate implied by the observed 2025 PPP baseline and this reconciled 2050 endpoint. The structural rate remains available as a separate audit field. The market-exchange-rate estimate allows gradual price-level convergence in lower-income economies, with that convergence reduced by volatility and resource risk. It does not assume that PPP and nominal GDP become equal.

Total GDP reconciles by construction:

`Total GDP = GDP per capita x projected population`

Population comes from the same UN WPP-linked national population series used elsewhere in the console. Economic strength is ranked by total nominal GDP; total PPP GDP, per-capita PPP, per-capita nominal GDP, and the broader resilience score remain separate measures. P10/P90 ranges vary the annual growth path using country-specific volatility, resource, and dependency risk.

The output preserves this constant-2021-dollar audit value and also reports a 2050-price-dollar view. The price conversion uses the BLS CPI-U annual-average change from 2021 to 2025 and compounds a 2% annual US-dollar inflation assumption from 2026 through 2050:

`PriceFactor_2021_to_2050 = (CPI_U_2025 / CPI_U_2021) x (1.02 ^ 25)`

The central factor is approximately 1.950. Low and high uncertainty paths use 1.5% and 3.0% annual inflation after 2025. Inflation-adjusted 2050 values are useful for future nominal scale, but are never passed back into the HDI income-index formula because doing so would confuse price growth with real human development. The estimates are independent scenarios, not official World Bank, IMF, UN, Federal Reserve, or government forecasts. Exchange rates, purchasing-power parities, inflation, borders, shocks, and institutions can diverge materially over a 25-year horizon.
